...
GeneveHandlerENI::~GeneveHandlerENI()
{
    ...
    // so the guard keeps that build correct.
    if(sendingSock >= 0)
        close(sendingSock);
    tunnelOut.reset();
#endif
    // Tear down the TUN devices explicitly (rather than relying on the implicit member-destructor calls
    // that would otherwise run after this body returns) so that, in netns mode, the namespace has nothing
    // left inside it by the time we remove it below.
    tunnelIn.reset();
    if(useNetns)
